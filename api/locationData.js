 ...
 * @description 전국 (제주도 포함) 지역 정보를 담는 데이터 파일의 구조를 정의합니다.
 * 이 파일은 기상청 날씨 API 연동, 사용자 입력 대응, 행정 계층 구조 및 우선순위 정렬을 목적으로 합니다.
 * 특정 지역의 위경도 데이터는 포함하지 않으며, `addLocation` 함수를 통해 외부에서 데이터를 추가할 수 있도록 설계되었습니다.
...
const locationData = (() => {
    ...
    // 우선순위 점수 매핑 (높을수록 중요)
    // 이 맵은 weather.js에서 지역 검색 시 우선순위를 부여하는 데 사용될 수 있습니다.
    // 여기에 정의된 우선순위는 addLocation 함수에서 특정 지역 데이터에 적용될 수 있습니다.
    const priorityMap = {
        '제주특별자치도': 1000,
        '제주시': 950,
        '서귀포시': 930,
    ...
    const addLocation = (locationObj) => {
        ...
        if (typeof lat !== 'number' || typeof lon !== 'number' || isNaN(lat) || isNaN(lon)) {
            // console.error(`Error: Invalid lat/lon for name '${name}'. Lat: ${lat}, Lon: ${lon}`); // 개발용 로깅, 최종본에서는 제거
            return;
        }
...
addLocation('삼동면', {lat: 35.5190861111111, lon: 129.159155555555, name: '울산광역시 울주군 삼동면', type: '면', admin_parent: '울산광역시 울주군', legal_divisions: ['하잠리, 금곡리, 보삼리, 조일리'], aliases: [], priority_score: priorityMap['삼동면']});
    ...
    // =============================================================
    // 이 섹션은 이제 비어있습니다.
    // 여기에 addLocation 호출을 통해 특정 지역 데이터를 추가할 수 있습니다.
    // 예시: addLocation({ name: '제주특별자치도', lat: 33.3616, lon: 126.5292, type: '광역자치단체', aliases: ['제주도'] });
    ...
    // 데이터 로드 확인용 통계 및 메타데이터 (현재는 데이터가 없으므로 초기값)
    const METADATA_INFO = {
        totalLocations: 0,
    ...
    return {
        ...
        findMatches: (query) => {
            ...
            for (const key in data) {
                ...
                // location.name이 없거나 유효하지 않은 경우 key를 사용 (에러 방지)
            ...
            return matches.sort((a, b) => {
                ...
                // 우선순위가 같으면 이름으로 정렬 (name 속성이 없으면 key 사용)
        ...
        findMatchingLocation: (coords) => {
            ...
            for (const key in data) {
                ...
                if (typeof loc.lat === 'number' && !isNaN(loc.lat) && typeof loc.lon === 'number' && !isNaN(loc.lon)) {
        ...
        findAllMatches: (searchTerm) => {
            const normalizedSearch = searchTerm.trim().toLowerCase().replace(/\s/g, ''); // 공백 제거 후 소문자 변환
            const matches = [];

            for (const key in data) {
                const location = data[key];
                const nameLower = (location.name || key).toLowerCase().replace(/\s/g, ''); // location.name이 없을 경우 key 사용
                const keyLower = key.toLowerCase().replace(/\s/g, '');

                // 이름, 키 또는 별칭, 법정동/리 목록에 검색어가 포함되는 경우
                if (nameLower.includes(normalizedSearch) ||
                    keyLower.includes(normalizedSearch) ||
                    (location.aliases && location.aliases.some(alias => alias.toLowerCase().replace(/\s/g, '').includes(normalizedSearch))) ||
                    (location.legal_divisions && location.legal_divisions.some(ld => ld.toLowerCase().replace(/\s/g, '').includes(normalizedSearch)))
                ) {
                    // 깊은 복사를 통해 원본 객체 변경 방지 및 `key`와 `priority` 추가
