...
/**
 * 지역명 문자열을 정규화합니다.
 * 공백 제거, 특수문자 제거 등을 수행하여 비교 정확도를 높입니다.
 * 이 함수는 주로 내부 비교 로직에 사용됩니다.
...
function normalizeLocationName(name) {
    ...
    // 이 함수는 비교를 위해 사용되므로, 최대한 불필요한 요소를 제거
    return name.toLowerCase()
        .replace(/\s+/g, '') // 모든 공백 제거
        .replace(/[^\w가-힣]/g, '') // 특수문자 제거 (한글, 영문, 숫자만 남김)
        // 행정구역 단위나 시설명은 포함 여부만 판단하므로 여기서는 제거하지 않음.
        // 예를 들어 '삼성동'에서 '동'을 제거하면 비교에 어려움 발생
        .trim();
}

/**
 * 전체 법정동/행정동명에서 가장 마지막 행정 단위(동, 읍, 면, 리 등) 또는
 * 마지막 구/시/군 단위를 추출하여 간결한 이름을 반환합니다.
 * 예: "서울특별시 강남구 삼성동" -> "삼성동"
 * 예: "제주특별자치도 제주시 애월읍" -> "애월읍"
 * 예: "서울특별시 강남구" -> "강남구"
 * 예: "서울" -> "서울"
 * @param {string} fullAddress - 전체 법정동/행정동명
 * @returns {string} 간결한 지역명
 */
function getSimplifiedLegalName(fullAddress) {
    if (!fullAddress) return '';
    const parts = fullAddress.split(' ');
    const lastPart = parts[parts.length - 1];

    // 마지막 부분이 '동', '읍', '면', '리', '가', '로' 등으로 끝나는 경우
    if (lastPart.endsWith('동') || lastPart.endsWith('읍') || lastPart.endsWith('면') || lastPart.endsWith('리') || lastPart.endsWith('가') || lastPart.endsWith('로')) {
        return lastPart;
    }
    // 마지막 부분이 '구', '시', '군' 등으로 끝나는 경우
    if (parts.length > 1 && (lastPart.endsWith('구') || lastPart.endsWith('시') || lastPart.endsWith('군'))) {
        return lastPart;
    }
    // 그 외의 경우 (예: '서울', '제주' 등)는 전체 주소 반환
    return fullAddress;
}
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        // 3. 지역명(region)으로 검색하는 경우 (가장 일반적인 경우, findLocationCoordinates 호출)
            ...
            let finalFullName = locationResult.fullLegalName; // Default to full legal name from coord2regioncode
            const matchedNameFromKakao = locationResult.matchedName; // Name from keyword/address search
            const simplifiedLegalName = getSimplifiedLegalName(locationResult.fullLegalName); // e.g., "삼성동" from "서울특별시 강남구 삼성동"

            // Logic to determine the best fullName to display
            const landmarkKeywords = ['역', '터미널', '공항', '병원', '점', '대'];
            const lowerCaseMatchedName = matchedNameFromKakao.toLowerCase();
            const lowerCaseQuery = region.toLowerCase();

            const isLandmarkMatchedName = landmarkKeywords.some(keyword => lowerCaseMatchedName.includes(keyword)) && lowerCaseMatchedName.includes(lowerCaseQuery);
            const isMatchedNameSignificantlyShorter = matchedNameFromKakao.length < locationResult.fullLegalName.length * 0.7; // e.g., '강남역' (3) vs '서울특별시 강남구 역삼동' (12)
            const isMatchedNameRelevantToQuery = lowerCaseMatchedName.includes(lowerCaseQuery) || lowerCaseQuery.includes(lowerCaseMatchedName);
            
            if ((isLandmarkMatchedName || isMatchedNameSignificantlyShorter) && isMatchedNameRelevantToQuery && locationResult.source === 'kakao_keyword') {
                finalFullName = matchedNameFromKakao;
                console.log(`💡 fullName 결정: 키워드 매칭명 우선 ('${finalFullName}')`);
            } 
            // Priority 2: If the query is a simplified legal name and it matches
            // Ensure simplified name is not too short (e.g., just '서' from '서귀포시')
            else if (normalizeLocationName(region) === normalizeLocationName(simplifiedLegalName) && simplifiedLegalName.length > 1) { 
                finalFullName = simplifiedLegalName;
                console.log(`💡 fullName 결정: 간소화된 법정동명 우선 ('${finalFullName}')`);
            }
            // Priority 3: Use the full legal name (default fallback)
            else {
                finalFullName = locationResult.fullLegalName;
                console.log(`💡 fullName 결정: 전체 법정동명 사용 ('${finalFullName}')`);
            }

            locationInfo = {
                requested: region,
                matched: locationResult.matchedName, // 카카오 API 키워드/주소 검색 결과명 또는 하드코딩 매칭명
                fullName: finalFullName, // 최종적으로 클라이언트에 보여줄 이름
                coordinates: coordinates,
                latLon: { lat: targetLat, lon: targetLon },
                ...
                simplifiedLegalName: simplifiedLegalName // 디버깅/추후 사용을 위해 간소화된 법정동명도 포함
            };
            console.log('🗺️ 지역명 검색 완료:', { 
                ...
                determinedFullNameLogic: finalFullName // 어떤 로직으로 fullName이 결정되었는지 로그
            });
        }
