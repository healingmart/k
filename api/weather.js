...
async function findLocationCoordinates(query) {
    ...
    const normalizedQueryForFallback = query.trim().toLowerCase();
    const exactFallbackMatchKey = Object.keys(fallbackLocations).find(key => key.trim().toLowerCase() === normalizedQueryForFallback);
    if (exactFallbackMatchKey) {
        console.log(`✅ 폴백에서 정확한 지역 매칭: '${query}' -> '${fallbackLocations[exactFallbackMatchKey].name}'`);
        return fallbackLocations[exactFallbackMatchKey];
    }
    ...
    if (KAKAO_REST_API_KEY) {
        try {
            ...
            if (kakaoResponse.data && kakaoResponse.data.documents && kakaoResponse.data.documents.length > 0) {
                ...
                // 문서에 점수를 매겨 최적의 일치 항목을 찾습니다.
                let bestScore = -1;
                let bestDoc = null;

                documents.forEach(doc => {
                    let score = 0;
                    const lowerCasePlaceName = doc.place_name.toLowerCase().trim();
                    const lowerCaseAddressName = doc.address_name ? doc.address_name.toLowerCase().trim() : '';

                    // 규칙 1: place_name이 쿼리와 정확히 일치 (최고 우선순위)
                    if (lowerCasePlaceName === lowerCaseQuery) {
                        score += 1000;
                    }

                    // 규칙 2: 행정구역 (AD5)은 매우 선호
                    if (doc.category_group_code === 'AD5') {
                        score += 500;
                    }
                    
                    // 규칙 3: place_name이 쿼리를 포함 (좋은 일치)
                    if (lowerCasePlaceName.includes(lowerCaseQuery)) {
                        score += 100;
                        // 정확히 일치하지 않으면서 쿼리를 포함하는 긴 이름에 페널티 부여
                        score -= (lowerCasePlaceName.length - lowerCaseQuery.length) * 0.1; 
                    }

                    // 규칙 4: address_name이 쿼리를 포함
                    if (lowerCaseAddressName.includes(lowerCaseQuery)) {
                        score += 50;
                    }

                    // 행정구역이 "시", "군", "구", "읍", "면", "동", "리"를 포함하는 경우 추가 점수 부여
                    if (doc.category_group_code === 'AD5') {
                        const adminKeywords = ['시', '군', '구', '읍', '면', '동', '리'];
                        const isSpecificAdminUnit = adminKeywords.some(keyword => lowerCasePlaceName.endsWith(keyword));
                        if (isSpecificAdminUnit) {
                            score += 200; // 명확한 행정 단위에 대한 점수 부스트
                        }
                    } else {
                        // 행정구역이 아니면서 정확히 일치하지 않는 장소에 페널티 부여 (예: "다인도자기"와 같은 상점)
                        if (lowerCasePlaceName !== lowerCaseQuery) {
                            score -= 20; 
                        }
                    }

                    // 쿼리와 주소가 정확히 일치하는 결과에 우선순위 부여
                    if (lowerCaseAddressName.includes(lowerCaseQuery) && lowerCaseAddressName.endsWith(lowerCaseQuery)) {
                         score += 150;
                    }
                    ...
                    if (score > bestScore) {
                        ...
                        // 점수가 같을 경우, AD5 유형 또는 더 짧은 place_name을 선호
                        if (bestDoc && doc.category_group_code === 'AD5' && bestDoc.category_group_code !== 'AD5') {
                            bestDoc = doc;
                        } else if (bestDoc && doc.category_group_code === 'AD5' && bestDoc.category_group_code === 'AD5') {
                            if (doc.place_name.length < bestDoc.place_name.length) {
                                bestDoc = doc;
                            }
                        }
                    }
                ...
                if (bestDoc) {
                    ...
                    console.log(`✨ Kakao Geocoding API: '${query}'에 대한 최적 결과 선택 (스코어 ${bestScore}): ${bestDoc.place_name}`);
                    return { lat, lon, name };
                } else {
                     console.warn(`⚠️ Kakao Geocoding API 결과 필터링 후 최적 결과 부재 for "${query}". 폴백 사용 시도.`);
                }
            } else {
    ...
    // 이 폴백은 명시적으로 문제가 있는 영역에 대해 중요합니다.
    const normalizedQueryForFallbackSearch = query.toLowerCase().trim()
        .replace(/\s+/g, '')
        .replace(/(시|군|구|읍|면|동|리)$/g, ''); // 일반적인 접미사 제거
    ...
    for (const [key, coords] of Object.entries(fallbackLocations)) {
        ...
        // 양방향으로 포함 여부 확인
        if (normalizedKey.includes(normalizedQueryForFallbackSearch) || normalizedQueryForFallbackSearch.includes(normalizedKey)) {
            console.log(`⚠️ 폴백 지역명으로 유사 매칭: '${query}' -> '${key}' (${coords.name})`);
            return coords;
        }
    }
    
    // 원래 정규화된 쿼리가 일치하지 않을 경우, 더 넓은 범위의 검색으로 폴백.
    // '수성1가'와 같이 '수성'만으로도 일치시킬 수 있는 경우를 처리합니다.
    const broadNormalizedQuery = query.toLowerCase().trim().replace(/\s+/g, '');
    for (const [key, coords] of Object.entries(fallbackLocations)) {
        const broadNormalizedKey = key.toLowerCase().trim().replace(/\s+/g, '');
        if (broadNormalizedKey.includes(broadNormalizedQuery) || broadNormalizedQuery.includes(broadNormalizedKey)) {
             console.log(`⚠️ 폴백 지역명으로 넓은 범위 유사 매칭: '${query}' -> '${key}' (${coords.name})`);
             return coords;
        }
    }
