 ...
 */
function getLocationCoordinates() { // 함수명 변경 제안 반영 (getFallbackLocationCoordinates -> getLocationCoordinates)
    return {
...
/**
 * Kakao Local API를 사용하여 위치 정보를 검색하는 함수.
...
async function searchLocationWithKakao(query, kakaoApiKey) {
    try {
        ...
        const keywordResponse = await axios.get('https://dapi.kakao.com/v2/local/search/keyword.json', {
            params: {
                query: query,
                category_group_code: '', // 모든 카테고리 검색
                size: 15, // 결과 개수 증가
                sort: 'accuracy' // 정확도 순
            },
            headers: {
        ...
        if (documents.length > 0) {
            // 2단계: 결과 필터링 및 우선순위 적용
            bestMatch = findBestLocationMatch(query, documents);
        }

        // 3단계: 키워드 검색 실패 시 주소 검색 시도 (필요시)
        if (!bestMatch || bestMatch.matchScore < 50) { // 키워드 매칭이 약할 경우 주소 검색 시도
...
function findBestLocationMatch(query, documents) {
    ...
    const scoredResults = documents.map(doc => {
        ...
        // 1. 완전 일치 (최고 점수)
        if (placeName === normalizedQuery) score += 200; // 장소명 완전 일치
        if (addressName === normalizedQuery) score += 180; // 주소명 완전 일치
        if (roadAddressName === normalizedQuery) score += 170; // 도로명 주소 완전 일치

        // 2. 행정구역 (AD5)에 대한 높은 가중치
        if (categoryGroupCode === 'AD5') {
            score += 100; // 행정구역인 경우 기본 점수
            // 행정구역이면서 쿼리가 포함될 경우 추가 점수
            if (placeName.includes(normalizedQuery) || addressName.includes(normalizedQuery)) {
                score += 50;
            }
        }
        ...
        if (placeName.includes(normalizedQuery)) {
            score += 40;
            // 검색어가 포함된 긴 이름에는 약간의 페널티 (너무 일반적인 장소명 방지)
            score -= (placeName.length - normalizedQuery.length) * 0.05; 
        }
        if (addressName.includes(normalizedQuery)) {
            score += 30;
            score -= (addressName.length - normalizedQuery.length) * 0.05; 
        }

        // 4. 카테고리별 추가 가중치 (필요에 따라 조정)
        if (doc.category_group_name === '관광명소') score += 20;
        if (doc.category_group_name === '교통,수송') score += 25; // 터미널, 역 등
        if (doc.category_group_name === '공공기관') score += 15;
        if (doc.category_group_name === '음식점') score += 10; // 음식점은 좀 더 낮은 우선순위

        // 5. 거리 기반 점수 (검색어와 떨어진 거리에 따라 페널티)
        // distance는 미터 단위, 너무 멀면 감점
        const distance = parseInt(doc.distance) || 0;
        if (distance > 0 && distance < 1000) score += 10;
        else if (distance >= 1000 && distance < 5000) score += 5;
        else if (distance >= 5000) score -= 10; // 5km 이상 떨어진 경우 감점

        return {
            ...doc,
            matchScore: score,
            normalizedName: placeName // 디버깅용
        };
    });
    ...
    // console.log(`🔍 '${query}'에 대한 카카오 검색 최고 점수 결과:`, bestResult);
...
function findBestAddressMatch(query, documents) {
    ...
    const scoredResults = documents.map(doc => {
        ...
        const addressName = normalizeLocationName(doc.address_name);
        const roadAddressName = normalizeLocationName(doc.road_address_name || '');
        const buildingName = normalizeLocationName(doc.building_name || '');

        // 1. 주소명 완전 일치
        if (addressName === normalizedQuery) score += 100;
        if (roadAddressName === normalizedQuery) score += 90;

        // 2. 주소명에 쿼리가 포함되거나 쿼리가 주소명에 포함
        if (addressName.includes(normalizedQuery)) score += 50;
        if (normalizedQuery.includes(addressName) && addressName.length > 2) score += 40;
        if (roadAddressName.includes(normalizedQuery)) score += 30;

        // 3. 건물명에 쿼리가 포함되는 경우 (참고용)
        if (buildingName.includes(normalizedQuery)) score += 10;

        // 주소의 깊이(depth)에 따른 가중치 (읍/면/동/리 등)
        if (doc.address && doc.address.region_3depth_name) score += 10;

        return {
            ...doc,
            matchScore: score,
            normalizedName: addressName
        };
    });
    ...
    const bestResult = scoredResults[0];
    // console.log(`🔍 '${query}'에 대한 카카오 주소 검색 최고 점수 결과:`, bestResult);

    // 최소 임계값 설정
    if (bestResult && bestResult.matchScore >= 40) { // 주소 검색은 좀 더 높은 신뢰도 필요
        return {
            lat: parseFloat(bestResult.y),
            lon: parseFloat(bestResult.x),
...
function normalizeLocationName(name) {
    ...
    // 특정 행정구역 단위나 일반적인 지명 접미사 제거 (필요에 따라 추가/수정)
    ...
    // 예외적으로 유지해야 할 특정 지명
    if (name.includes('터미널')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('역')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('공항')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('항구')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('항')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('해수욕장')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('해변')) normalized = name.toLowerCase().replace(/\s+/g, '');
    if (name.includes('센터')) normalized = name.toLowerCase().replace(/\s+/g, ''); // 여행자센터 등
...
async function findLocationCoordinates(query, kakaoApiKey) {
    ...
    const exactFallbackMatchKey = Object.keys(locations).find(key => 
        normalizeLocationName(key) === normalizedQuery ||
        normalizeLocationName(locations[key].name) === normalizedQuery
    );
...
module.exports = async function handler(req, res) {
    ...
    try {
        ...
        const kakaoApiKey = process.env.KAKAO_REST_API_KEY; // Kakao API 키 추가
