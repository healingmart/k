...
try {
    ...
    // latLonToGrid도 폴백으로 기본 서울 좌표 반환
...
async function handleWeatherRequest(req, res) {
    ...
    try {
        ...
        if (lat && lon) {
            ...
            if (!defaultLocation || Object.keys(locationData).length === 0) { // locationData가 로드되지 않은 경우도 처리
                // locationData가 없거나 기본 지역을 찾을 수 없을 때 샘플 데이터 반환하지 않고 에러 반환
        ...
        let responseData = {
            ...
            apiInfo: {
                ...
                apiKeyUsed: 'WEATHER_API_KEY',
                totalCategories: Object.keys(WEATHER_CODES).length,
                dataPoints: items.length, version: '2.0-complete'
            },
